...
To reduce merge conflicts when these CSVs change, `*.csv` files use Git's built-in `union` merge driver (see `.gitattributes`) and normalize to Unix line endings.
...
in two branches, you may still need to resolve it manually. The app now deduplicates users and site IDs at load time to avoid doubled counts after a union merge.
